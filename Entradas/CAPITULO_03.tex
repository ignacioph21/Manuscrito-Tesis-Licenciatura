\chapter{Sensor capacitivo} % REVISADO 1 (faltan gráficos)
\thispagestyle{plain} 

Este Capítulo está dedicado al modelado, diseño, construcción y caracterización del sensor capacitivo puntual que será utilizado en las experiencias de turbulencia de ondas, basado principalmente en el utilizado por Gordillo (2012) \cite{gordillozavaletaNonpropagatingHydrodynamicSolitons2012}. La hoja de ruta que fue utilizada para poder llevar a cabo el desarrollo del sensor se presenta en la Figura \ref{fig:timelinev2} y será la misma estructura que tendrá el Capítulo. % ´


\begin{figure}[th!]
	\centering
	\includegraphics[width=0.987\linewidth]{Figuras/Sensor/Timeline/Timeline} % _v2  
...
En la Sección \ref{sec:principio_funcionamiento} se presenta el modelado teórico del sensor y su principio de funcionamiento; en la Sección \ref{sec:circuito_numérico} la solución numérica de la electrónica asociada al sensor y la exploración del espacio de parámetros posibles para su construcción; en la Sección \ref{sec:circuito_protoboard} se construye un circuito esquemático equivalente para verificar que el comportamiento real se corresponda con los resultados numéricos; una vez verificado que el comportamiento del circuito equivalente sea el esperado se pasó a la construcción de un prototipo del sensor, que se presenta en la Sección \ref{sec:prototipado}; en la Sección \ref{sec:lock-in_digital} se explica el armado de un amplificador lock-in digital mediante la utilización de una placa de adquisición que permitirá tomar las mediciones; en la Sección \ref{sec:prueba_calibración} se caracteriza el sensor ya completado y; finalmente en la Sección \ref{sec:especificaciones_ténicas} se resumen las especificaciones finales que terminó teniendo el sensor construído. % á . 
...
A continuación se describe brevemente la técnica, mostrando  a modo ilustrativo en la Figura \ref{fig:ejemplo-v2} las señales involucradas en el proceso para un caso particular. El primer paso consistirá en modular la señal de interés, a la cual se llamará $A(t)$, con alguna frecuencia, obteniendo entonces una nueva señal % a y 
...
\begin{figure}[th!]
	\centering
	\includegraphics[width=0.650787\linewidth]{"Figuras/Sensor/Prueba protoboard/Datos_osciloscopio_unidades"}
...
\subsection{La sonda} 
Un modelo de la versión final de la sonda se muestra en la Figura \ref{fig:dibujosensormodelo}. Ésta es relativamente sencilla, siendo su componente principal el alambre de cobre esmaltado, que en este caso fue de $(0.35\pm0.05)$ mm de diámetro. Las partes restantes son básicamente para el soporte estructural del alambre, y para facilitar su conexión al circuito RLC. 

\begin{figure}[t!]
	\centering
	\includegraphics[width=0.87\linewidth]{"Figuras/Sensor/El sensor/dibujo_sensor_modelo"}
	\caption{Modelo 3D de la sonda construida. El componente principal es el alambre de cobre esmaltado de $(0.35\pm0.05)$ mm de diámetro. El soporte estructural consta de una carcasa impresa en PLA y un alambre de acero sin esmaltar que cumple la función adicional de conectar el agua a Tierra. } 
...
La parte superior de la carcasa se encuentra abierta, exponiendo así ambos alambres y permitiendo su conexión mediante cables de terminaciones tipo cocodrilo a la placa con el circuito RLC serie. Ésta sección permite además conectar la sonda, ya sea a un tornillo micrométrico que permita ajustar finamente la altura sumergida, o directamente a perfilería de aluminio si tal precisión no fuese necesaria. 
...
\begin{table}[t!] % h 
	...
	\begin{tabular}{|c|c|c|c|c|}
		\hline
		\textbf{Elemento} & R & L & C & C' \\ \hline
		\textbf{Valor} & $(360 \pm 40)\,\Omega$ & $(10 \pm 1)\,\text{mH}$ & $(560 \pm 60)\,\text{pF}$ &  $\sim\;1$ pF/mm \\ \hline
		  
	\end{tabular}
	\caption{Valores nominales de las componentes utilizadas en el circuito RLC serie del sensor capacitivo. } % a 
...
Con  este fin se utilizó Python 3, y se basó el desarrollo en el \textit{wrapper} de la biblioteca dinámica \textit{PyIOTech} \cite{PyIOTech}, que también fue necesario readaptar ya que se encuentra escrita en Python 2. 

No se entrara en detalle sobre el código en sí, pero el resultado final fue una interfaz gráfica de usuario mínima para llevar a cabo las mediciones en los canales deseados, que se encuentra disponible en \cite{ignacioph21_2026_18828702} y se introduce brevemente en el Apéndice \ref{sec:adquisición}. Se implementaron dos métodos de adquisición, en primer lugar uno para una cantidad de puntos predeterminados, ideal para mediciones cortas, y uno pensado para mediciones largas que cuenta con guardado en disco en simultáneo a la medición por bloques, más eficiente que el primero al utilizar un buffer circular pequeño y no necesitar guardar todos los puntos en memoria antes de escribirlos en disco. Éste segundo método fue el más utilizado ya que permite tomar datos continuamente y detenerlo cuando se considere suficiente. 
...
Usando el parámetro $\alpha=\cos(\gamma)-1+\sqrt{\cos^2(\gamma)-4\cos(\gamma)+3}$, que se relaciona con la frecuencia de corte, o el tiempo de integración $\tau_c$ de la forma: $\gamma=2\pi f_c/f_s$, con $f_s$ la frecuencia de sampleo. La segunda etapa es análoga a la primera: % uy l
...
Pudiendo  elegir trivialmente $X_0[-1]=Y_0[-1]=0$ al inicio de la señal. % Con este para la señal anterior y el FDF tenemos: Ypodemos 



El código para este lock-in digital se encuentra disponible en \cite{ignacioph21_2026_18828835}. La versión final permitió obtener resultados comparables a los de un Lock-in como el SR-830 (pudiendo llegar incluso a superar la frecuencia máxima de funcionamiento de éste según el caso), con todas las ventajas de portabilidad y escalado que vienen asociadas a la placa. Un ejemplo clásico que muestra el correcto funcionamiento de éste es la medida de la amplitud y fase de un RLC serie en función de la frecuencia, como el que se presenta en el Apéndice \ref{apéndice:prueba_lockin_digital}.   % muestra : \missingfigure{Código lock-in digital} 



\section{Prueba final y calibración} 
\label{sec:prueba_calibración} 
Teniendo ya construído el sensor, y puesta en marcha la técnica para la medición de la fase, se llevó a cabo la caracterización final y calibración del mismo, siendo este el paso final antes de utilizarlo en experiencias de turbulencia de ondas. 
...
El montaje para la calibración del sensor, como se muestra en la Figura \ref{fig:montajecalibracion},  consistió en un recipiente pequeño lleno de aproximadamente 5 cm de agua destilada, montado mediante un brazo de unos 15 cm de largo, impreso en 3D, a un motor lineal LinMot colocado verticalmente.

\begin{figure}[t!]
	\centering
	\includegraphics[width=0.87\linewidth]{Figuras/Setups/Montaje_calibracion_fondo}
	\caption{Modelado 3D del montaje utilizado para la calibración del sensor a escala real. Este se compone de un motor lineal colocado verticalmente (a) unido mediante un brazo (b) a un recipiente con agua destilada (c). El sensor (d) se encuentra fijo, sujeto a un perfil de aluminio mediante un tornillo micrométrico (e).} % tyt   
	\label{fig:montajecalibracion}
\end{figure}


Se alimentó al motor con una señal aleatoria periódica de 10 segundos de longitud (formada por ruido limitado en frecuencia entre 0-6 Hz y fase aleatoria), de forma tal que el movimiento fuese lo suficientemente lento como para poder considerar que la posición del motor es solidaria a la de la superficie libre, y evitar que se produzcan vibraciones en la misma. %  (Ondas de Faraday)
...
Se midió simultáneamente la fase del sensor, mediante la placa de adquisición y el lock-in digital, y la posición real del motor, mediante el software del controlador (vale aclarar que es la instantánea a la que llega, no los \textit{setpoints}  pedidos). 

Un par de observaciones importantes son que, si bien se intentó evitar al máximo las vibraciones en el \textit{setup} hubo algunas que podrían generar que el error aparente en las mediciones sean mayores a los reales, y además que, si la sonda solo se encuentra sumergida unos pocos mm parece no funcionar correctamente, con lo cual de aquí en más siempre se buscó que estuviese sumergida al menos un par de cm. % y  

\subsection{Respuesta lineal} % Altura vs Altura  
En la Figura \ref{fig:series-temporales-a} se muestran las series temporales obtenidas durante la medición. En \ref{fig:series-temporales-a}a se encuentra la posición real del motor, y debajo en \ref{fig:series-temporales-a}b la fase medida por el sensor. De la similitud entre ambas señales se puede intuir una relación que efectivamente sería lineal. 

\begin{figure}[t!]
	\centering
	\includegraphics[width=0.987\linewidth]{"Figuras/Sensor/Series temporales/Series temporales a"}
	\caption{(a) Posición real del motor en función del tiempo (solidaria a la posición de la superficie libre). (b) Fase medida por el sensor simultáneamente (el eje derecho muestra los valores para la señal calibrada por la función lineal obtenida de la Figura \ref{fig:motor-vs-fase}). (c) Diferencias de la altura real y la fase calibrada.} % ,  
	\label{fig:series-temporales-a}
\end{figure}

Para poder verificar esto se puede graficar una señal contra la otra, como en la Figura \ref{fig:motor-vs-fase}, y de observarse una tendencia lineal, como en este caso, realizar un ajuste lineal de los datos.  

\begin{figure}[t!]
	\centering
	\includegraphics[width=0.65560787\linewidth]{"Figuras/Sensor/Motor vs Fase/Motor vs FASE"}
	\caption{Posición real del motor (solidaria a la altura real de la superficie libre) contra la fase medida por el sensor simultáneamente (\markcircle*[blue]{0.4ex}). Se muestra además el ajuste lineal (\marksolid{0.4ex}) de parámetros a = $(604 \pm 3)$ mm/rad y $b = (1.97 ± 0.01)$ mm.}
...
%$a = (1.647062e-03 ± 7.2e-06)$ rad/mm
%a = (9.436972e-02 ± 4.1e-04) °/mm
%b = (-3.268623e-03 ± 8.6e-06) rad
%
%R² = 0.99518
%χ²_ν = 1.004
...
Se considera un muy buen ajuste, y que los datos siguen efectivamente una tendencia lineal durante todo el tiempo de la medición. Se puede apreciar de la pendiente que en principio se podrían mediar desplazamientos de hasta unos 6.0 cm antes de romper la hipótsis de fase pequeña. Por otro lado, la ordenada es distinta de cero, lo cual probablemente se deba a que la frecuencia del circuito estaba levemente desviada de la de resonancia. Además, puede verse muy poca dispersión en los puntos, con lo cual el ruido en la medición apuntaría a ser bajo. Tampoco pareciera haber histéresis en los datos.  



\subsection{Diferencias con la altura real} % au a   
Por último, se pueden usar los parámetros del ajuste lineal como parámetros de calibración del sensor, con lo cuales pasar de fase a altura (c.f. eje derecho de la Figura \ref{fig:series-temporales-a}). Una vez calibrada la señal se pueden calcular las dieferencias con la altura real, que sería el último panel de la Figura, \ref{fig:series-temporales-a}c.  

\begin{figure} % [!b] 
	\centering
	\includegraphics[width=0.987\linewidth]{Figuras/Sensor/Diferencias/Diferencias}
	\caption{(a) Histograma de las diferencias (Figura \ref{fig:series-temporales-a}c). (b) Espectro de las mismas diferencias que para (a).} 
	\label{fig:diferencias}
\end{figure}

A partir de estas diferencias, de dispersión 0.08 mm, se puede estimar un cota parar el error del sensor de aproximadamente 0.1 mm.  

Además, viendo el histograma de estas diferencias, Figura \ref{fig:diferencias}a, se puede notar que están centradas en 0 y decaen rápidamente. Se evaluó además la normalidad de esta distribución mediante el test de Lilliefors (con $N=256$ puntos), obteniendo valores de $D=0.061$ y $p=0.031<0.05$, con lo cual esto rechaza la hipótesis de normalidad al $5 \%$. Sumado a esto, se tiene una \textit{skewness} de $-0.116$ y una kurtosis de $-0.755$, con lo cual no presenta asimetrías marcadas asociadas a sesgos sistemáticos y tiene colas ligeramente más livianas que una distribución Gaussiana. 




Del espectro de las mismas, Figura \ref{fig:diferencias}b se ven oscilaciones a baja frecuencia pero que en principio no deberían ser relevantes en el cálculo de los espectros en turbulencia de ondas.  




\subsection{Ruido del sensor} 
En la Figura \ref{fig:espectro_ruido} se puede apreciar la densidad espectral de potencias del ruido del sensor, estando éste sumergido en un fluido estático. 

% \missingfigure{Espectro ruido}  

\begin{figure} % [t!]
	\centering
	\includegraphics[width=0.987\linewidth]{Figuras/Sensor/Ruido/espectro_ruido}
	\caption{Densidad espectral de potencias para el ruido del sensor (\marksolid[blue]{0.4ex}). Se marcan los múltiplos enteros de 50 Hz (\markdashed[black]{0.4ex}), que coincide con los picos. El leve decaimiento en las frecuencias más altas se debe al filtro pasabajos del algoritmo de demodulación homodina, específicamente a la elección de la frecuencia de corte de $f_c=1$ kHz. } % n e 
	\label{fig:espectro_ruido}
\end{figure}
...
\section{Especificaciones finales} % sub 77 
\label{sec:especificaciones_ténicas}  
El sensor construído es sencillo y de muy bajo costo, ya que se compone principalmente por un alambre de cobre esmaltado y otros componentes de fácil acceso. La única componente costosa necesaria para su utilización es la placa de adquisición (aunque ya de por sí es una alternativa mucho más económica que un amplificador Lock-in), pero para la cual podrían buscarse alternativas según sea lo que se desea medir, por ejemplo, mediante en placas de desarrollo (como son FPGA o Teensy). % a el por ejemplo 

De la Sección anterior se vio que el sensor puede llegar a resolver de base amplitudes de hasta aproximadamente 0.1 mm, aunque probablemente realizando una calibración más cuidadosa el error real sea menos.  

La frecuencia máxima a la que podrán medirse variaciones de $\Delta l$ estará relacionada a la frecuencia de resonancia del circuito RLC, ya que ésta será la moduladora, pudiendo llegar hasta algunas decenas de kHz con los componentes utilizados. Sin embargo, hay que tener en cuenta el hecho de que el alambre tiene un determinado diámetro, con lo cual no tendrá sentido caracterizar ondas con longitud de onda menor a $d=(0.35\pm0.05)$ mm. Utilizando la relación de dispersión para ondas gravito-capilares en el agua \eqref{eq:dispersion_relation} esta longitud de onda está asociada a una frecuencia $f=(3.3\pm0.7)$ kHz. Ésta será la frecuencia máxima a la cual tendría sentido medir con el sensor, que está por encima del 1 kHz hasta el cual desea utilizarse. % para , relación,

Con la placa de adquisicón utilizada es posible usar, hasta donde se probó, tres sensores simultáneamente, aunque sería escalable a más en la propia placa, y aún más utilizando múltiples placas sincronizadas. % Y c 
